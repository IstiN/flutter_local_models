    ...
    func generateVoiceDesign(
        ...
        if let conditioning {
           ...
           speechTokenizer.hasEncoder {
            ...
            var speaker: String? = isCVModel ? instruct : nil
            var effectiveInstruct: String? = isCVModel ? nil : instruct
            // Base checkpoints: Python passes CLI `--voice` as `speaker` into
            // `_prepare_generation_inputs` (codec spk embedding), not as a VoiceDesign-style
            // user-message `instruct`. Without this, preset names only worked for `custom_voice`.
            if config.ttsModelType == "base", let v = instruct, !v.isEmpty {
                speaker = v
